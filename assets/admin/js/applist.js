const App = Vue.createApp({
    ...
    methods: {
        AppDownload(id) {
            mdui.dialog({
                title: '温馨提示',
                content: '请选择需要执行的操作：<br>可直接获取下载地址，然后手动前往网站配置内填写！<br>也可选择直接将此App下载地址设置为网站App下载地址<br>注意：若<font color=red>站点编号</font>未绑定，则会绑定到主站，如果已经绑定，则会绑定至对应分店！',
                modal: true,
                history: false,
                buttons: [{
                    text: '关闭',
                }, {
                    text: '获取下载地址', onClick: function () {
                        App.Ajax(id, 1);
                    }
                }, {
                    text: '将下载地址绑定到网站', onClick: function () {
                        App.Ajax(id, 2);
                    }
                }],
                onOpen: function () {
            ...
            $.ajax({
                type: "POST", url: 'main.php?act=AppDownload', data: {
                    id: id, type: type,
                }, dataType: "json", success: function (res) {
                    layer.close(is);
                    if (res.code == 1) {
                        layer.alert(res.msg + '<br>' + res.url, {
                            btn: ['打开下载地址', '取消'], icon: 1, btn1: function () {
                                open(res.url);
                            }
                        });
                    } else {
            ...
            mdui.dialog({
                title: '温馨提示', content: '是否需要同步App打包任务进度？', modal: true, history: false, buttons: [{
                    text: '关闭',
                }, {
                    text: '确认同步', onClick: function () {
                        let is = layer.msg('同步中，请稍后...', {icon: 16, time: 9999999});
                        $.ajax({
                            type: "POST", url: 'main.php?act=AppCalibration', data: {
                                id: id
                            }, dataType: "json", success: function (res) {
                                layer.close(is);
                                if (res.code == 1) {
            ...
            mdui.dialog({
                title: '警告', content: '删除后，无法恢复，此App生成记录将完全消失，下载地址也将不可用！', modal: true, history: false, buttons: [{
                    text: '关闭',
                }, {
                    text: '确认删除', onClick: function () {
                        let is = layer.msg('删除中，请稍后...', {icon: 16, time: 9999999});
                        $.ajax({
                            type: "POST", url: 'main.php?act=AppDelete', data: {
                                id: id
                            }, dataType: "json", success: function (res) {
                                layer.close(is);
                                if (res.code == 1) {
            ...
            mdui.dialog({
                ...
                buttons: [{
                    ...
                    text: '确认提交', onClick: function () {
                        let is = layer.msg('加载中，请稍后...', {icon: 16, time: 9999999});
                        $.ajax({
                            type: "POST", url: 'main.php?act=AppSubmit', data: {
                                id: id
                            }, dataType: "json", success: function (res) {
                                layer.close(is);
                                if (res.code == 1) {
            ...
            mdui.prompt('要改成什么？,请在下方填写', msg, function (str) {
                ...
                $.ajax({
                    type: "POST", url: 'main.php?act=AppSet', data: {
                        id: id, field: field, value: str,
                    }, dataType: "json", success: function (res) {
                        layer.close(is);
                        if (res.code == 1) {
                            layer.alert(res.msg, {
...
<div class="mdui-textfield">
  <label class="mdui-textfield-label">用户ID，可填写-1,不绑定任何用户</label>
  <input class="mdui-textfield-input" id="uid" type="text"/>
</div>
                `;
            mdui.dialog({
                title: 'App生成任务创建',
                content: content + '<hr>Ps：任务创建成功后，可点击对应内容修改数据！',
                modal: true,
                history: false,
                buttons: [{
                    text: '关闭',
                }, {
                    text: '确认生成', onClick: function () {
                        let is = layer.msg('生成中，请稍后...', {icon: 16, time: 9999999});
                        $.ajax({
                            type: "POST", url: 'main.php?act=AppAdd', data: {
                                'name': $("#name").val(), 'url': $("#url").val(), 'uid': $("#uid").val(),
                            }, dataType: "json", success: function (res) {
                                layer.close(is);
                                if (res.code == 1) {
                                    layer.alert(res.msg, {
            ...
            layer.open({
                ...
                btn1: function () {
                    ...
                    $.ajax({
                        type: "POST", url: '../ajax/ajax.php?act=CreateAppSubmit', data: {
                            id: id,
                        }, dataType: "json", success: function (res) {
                            layer.close(is);
                            if (res.code >= 0) {
            ...
            let _this = this;
            let content = `
                <div style="text-align: center"><img src="./main.php?act=AppImage&id=` + image + `" style="width:100px;min-height:100px;max-height:200px;" /></div><hr>
            ...
            layer.open({
                ...
                success: function () {
                    _this.upload.render({
                        elem: '#Image',
                        url: './main.php?act=AppUploading&id=' + id + '&type=' + type,
                        size: 1024 * 2,
                        accept: 'images',
                        acceptMime: 'image/*',
                        exts: 'jpg|jpeg|png',
                        before: function () {
                            tipId1 = layer.msg('正在上传中...', {
                                icon: 16, shade: 0.01, time: 9999999
                            });
                        },
                        done: function (res) {
                            layer.alert(res.msg, {
            ...
            layer.open({
                ...
                btn1: function () {
                    ...
                    $.ajax({
                        type: "POST", url: './main.php?act=AppColorSet', data: {
                            id: id, color: colorset, type: type,
                        }, dataType: "json", success: function (res) {
                            layer.close(is);
                            if (res.code >= 0) {
                                layer.alert(res.msg, {
            ...
            $.ajax({
                type: "POST", url: './main.php?act=AppList', data: {
            ...
            mdui.prompt('可输入：用户ID，App名称,ID等', '搜索任务', function (str) {
            ...
            layui.use('laypage', function () {
                ...
                $.ajax({
                    type: "POST", url: './main.php?act=AppCount', data: {
