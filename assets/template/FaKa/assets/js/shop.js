const vm = Vue.createApp({
	data() {
		return {
			...
			}, num: 1, Price: 0, SKU: [], //当为商城模式时此处生效
			SkuType: -1, form: [], js: ``, Service: [], //客服
			InformData: {
				Navigation: []
			}, //公告
			SubData: [], //订单信息
		...
		CouponSelectR(index) {
			...
			let _this = this;
			$.ajax({
				type: 'post', url: './main.php?act=inform', dataType: 'json', success: function (data) {
					if (data.code == 1) {
			...
			layer.alert('<center><h3>客服QQ：' + this.Service.qq + '</h3><br><img src="' + this.Service.image + '" width="300" height="300" /><br><hr>' + this.Service.tips + '</center>', {
				title: '联系客服',
				btn: BtnArr,
				area: ['auto', '80%'],
				shade: [0.8, '#393D49'],
			...
			let _this = this;
			$.ajax({
				type: 'post', url: './main.php?act=Service', dataType: 'json', success: function (data) {
					if (data.code == 1) {
						_this.Service = data;
					} else {
						layer.msg(data.msg, {
			...
			$.ajax({
				type: "POST",
				url: './main.php?act=CreateOrder',
				data: thie.SubmitData(),
				dataType: "json",
				success: function (res) {
					layer.close(is);
					if (res.code >= 0) {
						data = res.data;
						if (res.Coupon.length >= 1 || res.CouponOff.length >= 1) {
							thie.CouponOn = res.Coupon;
							thie.CouponOff = res.CouponOff;
			...
			let is = layer.msg('加载中，请稍后...', {icon: 16, time: 9999999});
			$.ajax({
				type: "POST", url: './main.php?act=Pay', data: Data, dataType: "json", success: function (data) {
					layer.close(is);
					if (data.code == 1) {
						_this.queue();
						layer.alert(data.msg, {
							title: '通知', icon: 1, btn: ['确定'], yes: function () {
