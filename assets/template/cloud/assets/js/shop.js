			...
			axios.get("./main.php?act=Goods&gid=" + this.gid).then((res) => {
				...
				if (data.code >= 0) {
					...
					if (data.data.level_arr.length == 1) {
						layer.tips('<font color="white">恭喜！您当前为最高等级密价！</font>', "#level", {
							tips: [1, "#651FFF"], zIndex: 1
						});
						...
						layer.tips('<font color="white">此商品有更多优惠哦！<br>点击查看获取！</font>', "#level", {
							tips: [3, "#6200EA"], zIndex: 1
						});
					...
					layui.use("util", function () {
						...
						util.fixbar({
							bar1: "&#xe603;", click: function (type) {
								if (type === "bar1") {
									location.href = "./?cid=" + data.data.cid;
								}
							},
						});
					});
