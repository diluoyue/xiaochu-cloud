function hiset() {
    var display = $('.qt-header').css('display');
    if (display == 'none') {
        $('.qt-header').show(200);
        $("#sicon").attr('class', 'layui-icon layui-icon-up');
    } else {
        $('.qt-header').hide(200);
        $("#sicon").attr('class', 'layui-icon layui-icon-search');
    }
}
    ...
    methods: {
        OrderModification(oid, order, input) {
            ...
            for (const arrKey in arr) {
                ...
                `;*/
                content += `<div class="layui-form-item"><label class="layui-form-label" style="width: 100%; text-align: left; padding: 0px;">` + str[0] + `</label><div class="layui-input-"><input type="text" id="inputs_` + arrKey + `" value="` + str[1] + `" type="text" class="layui-input"></div></div>`;
            }
            layer.open({
                title: false,
                area: ['350px', '400px'],
