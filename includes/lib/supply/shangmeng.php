...
class shangmeng
{
    ...
    {
        if ($Data['type'] != 2 && !empty($_SESSION['MengSwGoodsList_' . $Data['id']])) {
            $GoodsList = [];
            foreach ($_SESSION['MengSwGoodsList_' . $Data['id']] as $val) {
                $GoodsList[] = [
                    'gid' => $val['id'],
        ...
        if ((int)$CurlDataJson['code'] === 1000) {
            ...
            $_SESSION['MengSwGoodsList_' . $Data['id']] = $CurlDataJson['data'];
